class BV
  class TestNode < Test::Unit::TestCase
    ...
    sub_test_case "Lambdaクラス" do
      test "assignable_exp_maxは1であること" do
        assert_equal 1, Node.get(:lambda).assignable_exp_max
      end

      test "lambdaはもっていない" do
        assert_equal false, Node.get(:lambda).has_lambda?
      end

      test "push_exp後のto_aは[:lambda [id] e1]の形式であること" do
    ...
    sub_test_case "TFoldクラス" do
      test "assignable_exp_maxは1であること" do
        assert_equal 1, Node.get(:tfold).assignable_exp_max
      end

      test "lambdaはもってない" do
        # TFoldは特別扱い
        assert_equal false, Node.get(:tfold).has_lambda?
      end

      test "push_expのto_aは[:lambda, [:x], [:fold, :x, 0, [:lambda, [:x, :y], e1]]]の形式であること" do
        node = Node.get(:tfold)
    ...
    sub_test_case "Foldクラス" do
      ...
      test "push_exp後のto_aは[:fold, e1, e2, [:lambda, [id, id], e3]]の形式であること" do
        node = Node.get(:fold)
        true while node.push_exp(0)
      ...
      test "tree_sizeは子供の分のサイズも合計して返す" do
        root = Node.get(:tfold)
    ...
    sub_test_case "OP1クラス" do
      ...
      test "push_exp後のto_aは[op, e1]の形式であること" do
        node = Node.get(:shr16)
